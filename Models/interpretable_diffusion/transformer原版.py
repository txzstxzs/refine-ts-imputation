...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, timestep,  mask=None, label_emb=None):
        ...
        a, att = self.attn1(self.ln1(x, timestep,  label_emb), mask=mask)     # 这四行就是一个transformer  两个注意力 一个交叉注意力 融入编码器信息
        x = x + a
...
class Transformer(nn.Module):
    ...
    def forward(self, input, t,  padding_masks=None):
        ...
        enc_cond = self.encoder(inp_enc, t,  padding_masks=padding_masks)  # 输入编码器 监督信息
