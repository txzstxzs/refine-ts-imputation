...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.rawdata, self.scaler = self.read_data(data_root, self.name)   # 读取原数据表格未归一化  
        ...
        self.data = self.__normalize(self.rawdata)                  # 输入表格 归一化 
        train, inference = self.__getsamples(self.data, proportion, seed)  # 归一化后的表格数据进行滑动窗口分割
    ...
    def __getsamples(self, data, proportion, seed):
        ...
        if self.save2npy:
            ...
            np.save(os.path.join(self.dir, f"{self.name}_ground_truth_{self.window}_train.npy"), self.unnormalize(train_data)) # 反平移反归一化放为原数据
    ...
    '对把三维数据转换表格形式 再进行最小最大归一化  后面还进行了平移'
